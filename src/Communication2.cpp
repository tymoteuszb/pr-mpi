...
#define TAG_GROUP_CREATED 4
#define TAG_GROUP_RESOLVED 5
...
void Communication::run() {
	...
	struct requestData recvRequestData[6];
	...
	MPI_Irecv(&recvGroupInfoData, 1, this->mpi_group_info_type, MPI_ANY_SOURCE, TAG_OPEN_FREE, MPI_COMM_WORLD, &recvGroupInfo);
	...
	switch(tag) {
		...
		case TAG_DIE_REQUEST:
			...
			{	
				...
				this->printMe(); cout << "ja wciaz pije, a moja grupe opuscil " << sender.id << endl;
			} 
			...
			else if (this->localStatus == 3 && inMyGroup(sender.id)) 
			{
				if (this->mpiRank > sender.id) 
				{
					...
					// Wyrzuć wysyłającego z grupy
					this->myGroup[sender.id] = false;
					this->printMe(); cout << "padlem juz, a moja grupe opuscil " << sender.id << ", czy zostalem sam w grupie? " << this->MyGroupEmpty() << endl;
		...
		case TAG_GROUP_RESOLVED:
			...
			this->printMe(); cout << "proces " << sender.id << " zwolnil arbitra, czy czekam na jakiegos? " <<  this->waitingForArbiter << endl;
			// Zwolniaj arbitra
			this->arbiters++;
